...
def compute_winner(s: str, boost: int) -> int:
    a1_s, a2_s = s.split('\n\n')
    a1 = [Group.parse(s) for s in a1_s.splitlines()[1:]]
    a1 = [g._replace(attack=g.attack + boost) for g in a1]
    ...
    while a1 and a2:
        ...
        for g_priority, targets, this_l, other_l in all_things:
            if g_priority in targets:
                ...
                units_before = other_l[ind].units
                other_l[ind] = other_l[ind].receive_attack(g)
                if other_l[ind].units != units_before:
                    did_any_damage = True

        if not did_any_damage:
            raise Lost()
